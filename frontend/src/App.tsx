...
type LearningFactor = { key:string; label:string; games:number; wins:number; accuracy:number; lift:number; reliable:boolean; signal:"positive"|"negative"|"neutral" };
type SelfLearning = { enabled:boolean; status:string; graded_games:number; minimum_games:number; progress_percent:number; active:boolean; probability_correction:number; baseline_accuracy:number; average_confidence:number; calibration_gap:number; factors:LearningFactor[]; recommendations:string[]; updated_at:string|null; safety:{method:string;maximum_adjustment_points:number;core_weights_changed:boolean} };
type Performance = { summary: { total_predictions:number; graded_predictions:number; pending_predictions:number; wins:number; losses:number; accuracy:number; units:number; roi:number; current_streak:number; streak_type:string|null }; confidence_tiers:{tier:string;predictions:number;wins:number;accuracy:number}[]; trend:{date:string;accuracy:number;predictions:number;cumulative_units:number}[]; team_performance:{team:string;predictions:number;wins:number;accuracy:number}[]; calibration:{bucket:string;predictions:number;expected:number;actual:number}[]; recent:HistoryItem[]; self_learning?:SelfLearning; };
  ...
  function renderPerformance() {
    ...
    return <>
      ...
      {loadingPerformance ? <LoadingCard text="Grading predictions and calculating performance…" /> : !performance ? <EmptyState title="No performance data yet" text="Run predictions, then return after games become final." /> : <>
        ...
        {performance.self_learning && <section className="panel"><div className="learning-header"><div><p className="eyebrow">SELF-LEARNING AI</p><h3>{performance.self_learning.status}</h3><p className="muted">Studies graded predictions and applies a conservative calibration correction after {performance.self_learning.minimum_games} completed games.</p></div><span className={`learning-badge ${performance.self_learning.active?'active':'collecting'}`}>{performance.self_learning.active?'ACTIVE':`${performance.self_learning.progress_percent.toFixed(0)}% READY`}</span></div><div className="stats-grid learning-stats"><StatCard label="Graded sample" value={`${performance.self_learning.graded_games}`} note={`Minimum ${performance.self_learning.minimum_games}`}/><StatCard label="Calibration gap" value={`${performance.self_learning.calibration_gap>=0?'+':''}${performance.self_learning.calibration_gap.toFixed(1)} pts`} note="Actual minus expected" positive={Math.abs(performance.self_learning.calibration_gap)<3}/><StatCard label="Learned correction" value={`${performance.self_learning.probability_correction>=0?'+':''}${performance.self_learning.probability_correction.toFixed(1)} pts`} note={`Capped at ±${performance.self_learning.safety.maximum_adjustment_points}`}/><StatCard label="Core weights" value="Protected" note="No automatic weight rewriting" positive/></div><div className="learning-grid"><div><p className="eyebrow">FACTOR REPORT</p><div className="calibration-list">{performance.self_learning.factors.slice(0,6).map(factor=><div className="calibration-row" key={factor.key}><strong>{factor.label}</strong><span>{factor.accuracy.toFixed(1)}% · {factor.games} games</span><span className={factor.lift>=0?'positive':'negative'}>{factor.lift>=0?'+':''}{factor.lift.toFixed(1)} lift</span></div>)}</div></div><div><p className="eyebrow">MODEL TAKEAWAYS</p><div className="limitation-list">{performance.self_learning.recommendations.map(item=><div key={item}><span>↗</span><p>{item}</p></div>)}</div></div></div></section>}
      </>}
    </>;
  }
