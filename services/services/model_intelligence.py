...
def build_model_intelligence(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    available_factors = [item for item in factors if item["available"]]
    total_strength = sum(item["strength"] for item in available_factors) or 1.0
    prediction_dna = []
    for item in sorted_factors:
        if not item["available"]:
            continue
        aligns_with_pick = item["favored_team"] == winner
        is_counter = item["favored_team"] == loser
        share = round((item["strength"] / total_strength) * 100, 1)
        if aligns_with_pick:
            role = "support"
            impact = "Primary driver" if item is top_support else "Supporting edge"
        elif is_counter:
            role = "risk"
            impact = "Primary counterweight" if item is top_counter else "Opposing signal"
        else:
            role = "neutral"
            impact = "Neutral signal"
        prediction_dna.append({
            "name": item["name"],
    ...
    supporting_strength = sum(item["strength"] for item in winner_factors)
    opposing_strength = sum(item["strength"] for item in loser_factors)
    signal_total = supporting_strength + opposing_strength
    alignment = round((supporting_strength / signal_total) * 100, 1) if signal_total else 50.0
    conflict = round(100 - alignment, 1)
    conviction = round(_clamp((winner_probability - 50) * 2 + (alignment - 50) * 0.35, 0, 100), 0)
    balance_label = "Unified" if alignment >= 75 else "Leaning" if alignment >= 60 else "Conflicted"

    dna_summary = (
        f"{alignment:.0f}% of the measured factor strength points toward {winner}. "
        f"The signal profile is {balance_label.lower()}, led by {support_name}; "
        f"{counter_name} remains the strongest opposing force."
    )
    ...
    return {
        ...
        "prediction_dna": {
            ...
            "dominant_driver": top_support["name"] if top_support else "No dominant driver",
            "counterweight": top_counter["name"] if top_counter else "No major counterweight",
            "summary": dna_summary,
            "components": prediction_dna,
            "flip_conditions": [
                f"A material downgrade to {winner}'s probable starter or confirmed lineup.",
                f"Unexpected bullpen unavailability that strengthens {loser}'s late-game path.",
                f"Market or weather information that meaningfully reduces the current {edge:.1f}-point edge.",
            ],
        },
        "risk": {
